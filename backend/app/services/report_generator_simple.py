...
def _gen_section1(basic: dict, analysis: dict, name: str, gender: str, version: str) -> list:
    ...
    lines.append('> 简而言之，您先天命局以' + ge_ju_str + '为主体架构，' + ('身虽弱但格局清奇，善借外力方可成事' if '偏弱' in sq_desc else sq_desc) + '，人生基调由此奠定。')
    lines.append("")
    ...
    lines.append("")
    lines.append(f'> 【金鉴真人·§1·身强弱规则】 {ri_gan}命主身强弱判定基于月令旺衰、地支根气、印比生扶三要素综合评分。本项采用【金鉴真人·§1·身强弱规则】，得分{sq_score}分，评定为「{sq_level}」。{"身弱需借火土之力方可成事" if sq_desc.startswith("命主自身能量偏弱") else sq_desc}')
    lines.append("")
...
def _gen_section2(basic: dict, analysis: dict) -> list:
    ...
    # 维度一：主格（核心格局）
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    lines.append('### 12.1 主格分析（月令定核心格局）')
    lines.append('')
...
def _gen_section4(basic: dict, analysis: dict) -> list:
    ...
    for ji in ji_list[:3]:
        ...
        if ji == "官杀":
            ...
            if wx_actual == "水":
                problem = "投资风险/资金外流"
                caution = "水旺年份控制负债和风险敞口"
            elif wx_actual == "木":
                problem = "职场压力/人际关系"
                caution = "木旺年份宜守不宜攻，避免重大职业变动"
            elif wx_actual == "金":
                problem = "口舌是非/合约风险"
                caution = "金旺年份签合同需逐条审核"
            else:
                problem = "能量失衡"
                caution = "注意相应五行层面的平衡"
    ...
    for ji in ji_list[:3]:
        ...
        if ji == "官杀":
            ...
            if wx_actual == "水":
                lines.append(f"- 忌神为{ji}（{wx_actual}）：水旺年份（如壬申/癸酉年）投资理财需谨慎，易有资金外流风险。")
            elif wx_actual == "木":
                lines.append(f"- 忌神为{ji}（{wx_actual}）：木旺年份（如甲寅/乙卯年）职场压力增大，注意人际关系，避免与上级冲突。")
            elif wx_actual == "金":
                lines.append(f"- 忌神为{ji}（{wx_actual}）：金旺年份（如庚申/辛酉年）口舌是非较多，谨言慎行，合同条款要仔细。")
            else:
...
def _gen_section11(basic: dict, analysis: dict, birth_year: int) -> list:
    ...
    if grade.startswith("高学历"):
        ...
        yun_str2 = '、'.join(yun_list2) if yun_list2 else '后续印星大运'
        ...
        yun_str = '、'.join(yun_list) if yun_list else '后续行印运的年份'
        lines.append(f"建议在印比大运期间（{yun_str}）重点发力，这是提升学历的最佳窗口。")
...
def _gen_section12(basic: dict, analysis: dict) -> list:
    ...
    if window_years:
        ...
        xi_dy_str_m = '、'.join(xi_dy_with_age[:4]) if xi_dy_with_age else '后续喜用神运'
        if _current_age > 35:
            lines.append(f"您已进入中年（{_current_age}岁），婚姻缘分已在喜用神大运期间引动。您的{xi_dy_str_m}为喜用神运，婚姻缘分在该时期有良好引动条件。")
        else:

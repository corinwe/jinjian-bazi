...
from app.models import User, Report, ReportVersion
...
def run_full_analysis_v4(name: str, gender: str, birth_year: int,
                          birth_month: int, birth_day: int,
                          birth_hour: int, calendar: str = "solar") -> dict:
    """排盘+完整分析（支持阳历/农历）"""
...
class AnalyzeRequest(BaseModel):
    year_gan: str; year_zhi: str; month_gan: str; month_zhi: str
    day_gan: str; day_zhi: str; hour_gan: str; hour_zhi: str
    gender: str; birth_year: Optional[int] = 1980; birth_month_lunar: Optional[int] = 1


@router.post("")
@router.post("/date")
def analyze(data: AnalyzeByDateRequest, user: User = Depends(get_current_user),
    ...
    version = ReportVersion(
        report_id=report.id, version="4.0",
        result_json=json.dumps(result, ensure_ascii=False),
...
def polish_report(report_id: int,
                   db: Session = Depends(get_db)):
    """LLM润色报告（生成自然语言报告）"""
...
def _polish_with_llm(data: dict) -> str:
    ...
    lines.append("月令印星: " + str(s['details']['yue_ling_yin']) + "分 | 月令比劫: " + str(s['details']['yue_ling_bi_jie']) + "分")
    lines.append("天干比劫: " + str(s['details']['tian_gan_bi_jie']) + "分 | 地支印比: " + str(s['details']['ri_zhi_yin_bi']) + "+" + str(s['details']['nian_shi_zhi_yin_bi']) + "分")
    lines.append("")
    lines.append("## 财星分析")
    lines.append("财星总分: **" + str(c['total']) + "分**（月令" + str(c['details']['yue_ling']) + " + 日支" + str(c['details']['ri_zhi']) + " + 时干" + str(c['details']['shi_gan']) + " + 时支" + str(c['details']['shi_zhi']) + " + 年支" + str(c['details']['nian_zhi']) + "）")
    lines.append("")
    lines.append("## 格局判定")
    ...
    if best:
        lines.append("**最佳大运**: " + best['gan_zhi'] + "（" + str(best['start_age']) + "~" + str(best['end_age']) + "岁）- " + str(best['score']) + "/10")
    if worst:
        lines.append("**最差大运**: " + worst['gan_zhi'] + "（" + str(worst['start_age']) + "~" + str(worst['end_age']) + "岁）- " + str(worst['score']) + "/10")
    lines.append("")
    lines.append("## 人生维度评分")

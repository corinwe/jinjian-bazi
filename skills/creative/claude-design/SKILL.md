...
- **brand identity designer for logos, naming, and visual systems**
...
- **logo / brand identity** — naming, logo variants (SVG), color palette, avatar, usage scenarios
...
When the user asks for a logo, brand name, or visual identity, load `references/logo-design-workflow.md` alongside this SKILL.md. That reference covers the full naming→visual→delivery pipeline, SVG technical patterns, palette guidance, and pitfalls specific to brand work.

Default deliverable for logos: 3 SVG variants (traditional, modern, symbolic) + 1 circular avatar SVG + 1 HTML brand identity showcase page.
...
- **for logo/brand design: hand-craft SVGs as the primary format** — scalable, editable, render everywhere. See `references/logo-design-workflow.md` for SVG logo patterns (gradients, seals, Chinese typography). This is the **recommended fallback** when `image_generate` is unavailable.
...
- **Do not execute visual design (logo, brand, UI) without first clarifying the user's style preference.** Logo/brand identity is highly subjective—always ask: "What style do you prefer? Minimalist and modern? Classic and traditional? Bold and colorful?" before producing any visuals. A design the user hates wastes everyone's time. For personal brand work especially, the user's aesthetic preference MUST be established before any artifact is created.

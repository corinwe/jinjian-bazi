...
## 🚨 铁律：8字段索引规则（2026-07-16 主母月令Bug校准）

> **根因**：`G = [B8[0], B8[2], B8[4], B8[6]]` → 取成了[年干,日干,年支,日支] ❌
> **结果**：主母月令被取成"壬"（时干位置），实际应为"未"（月支位置）。
> **所有报告都用了这个错误索引**，直到主母报告被老板指出才修。

```python
B8 = DS['8字段']  # [年干,月干,日干,时干,年支,月支,日支,时支]

# ✅ 正确
G = B8[0:4]  # [年干,月干,日干,时干]
Z = B8[4:8]  # [年支,月支,日支,时支]

# ❌ 错误（旧代码 2026-07 全月错误）
G = [B8[0], B8[2], B8[4], B8[6]]  # ← 年干日干年支日支，全乱！
Z = [B8[1], B8[3], B8[5], B8[7]]  # ← 月干时干月支时支，全乱！
```

**验证方法**：建立报告后立即检查月令是否正确。
```python
assert Z[1] == DS['八字'].split()[1][1], f'月支应为{DS[\"八字\"].split()[1][1]}, 实际为{Z[1]}'
```

## 🚨 铁律：十神阴阳判定规则（2026-07-16 子源十神Bug校准）

> **根因**：`calc_shishen()` 中同阴阳/异阴阳规则倒置。
> 错误：`'偏印' if tg_yy != rz_yy else '正印'` → 异阴阳→偏印 ❌
> 正确：`'偏印' if tg_yy == rz_yy else '正印'` → 同阴阳→偏印 ✅
...
口诀：**同阴阳=偏的/凶的（偏印、七杀、劫财、伤官、偏财）**
　　　**异阴阳=正的/吉的（正印、正官、比肩、食神、正财）**

### 示例（子源Bug）

子源：日主丙火(阳火)，年支卯藏乙木(阴木)
- 乙木生丙火：生我=印星，异阴阳 → **正印** ✅（旧代码错判为偏印）
- 若为甲木生丙火：同阴阳 → 偏印 ✅

主母成：日主庚金(阳金)，月支未藏丁火(阴火)
- 丁火克庚金：克我=官杀，异阴阳 → **正官** ✅（旧代码错判为七杀）

...
def gen(ds_path, label):
    ...
    if '身强' in shen_lv: a('身强不喜印。18岁前走印运不利学业。')
        ...
        if r == '丙' and ds['月支'] == '巳':
            a('中和偏旺。18岁前走水运(壬辰)调候则学业顺，走木火运反不利。')
        else:
            a('中和格局，学业看大运。')
    ...
    # 根据五行喜用给出具体推荐
    if r == '辛':  # 魏启令 - 身强辛金
        a('喜用五行：木(财) > 火(官杀) > 水(食伤)')
        a('① 绿檀手串（木·绿）— 财星喜用，招财养神')
        a('② 朱砂手串（火·红）— 官杀喜用，助事业')
        a('③ 黑曜石手串（水·黑）— 食伤喜用，利创作')
        a('忌：金发晶、银饰、白水晶（金旺比劫争财）')
    elif r == '庚':  # 主母成 - 从弱庚金
        a('喜用五行：木(财) > 火(官杀)')
        a('① 绿檀手串（木·绿）— 财星喜用，招财')
        a('② 红玛瑙手串（火·红）— 官杀喜用，稳家庭')
        a('③ 紫檀手串（木·紫红）— 木火兼得')
        a('忌：金银首饰、黄玉/蜜蜡（金土为忌）')
    elif r == '丙':  # 子源 - 丙火巳月
        a('喜用五行：水(调候) > 土(泄火) > 金(财星)')
        a('① 海蓝宝/黑曜石手串（水·蓝/黑）— 调候制火，最利')
        a('② 蜜蜡/黄玉手串（土·黄/咖）— 泄火生金')
        a('③ 金发晶手串（金·白）— 财星，利学业')
        a('忌：绿檀、红玛瑙等木火材质（火上浇油）')
    else:
        a('① 绿檀手串（木·绿）② 朱砂手串（火·红）③ 黑曜石（水·黑）')
    a('')

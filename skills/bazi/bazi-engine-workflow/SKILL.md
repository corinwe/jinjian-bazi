...
## ✅ 引擎Bug已修复：年柱立春前错误（2026-06-24发现 → 2026-09-10根治）
> 魏齐（2020-01-28 戌时）旧引擎输出庚子年，正确为**己亥年**（立春前归上一年）。
> **2026-09-10 已根治**：统一走 `engine/jieqi.py`（lunar-python精确节气，立春精确到分钟）——
> `jieqi.year_gan_zhi(dt)` / `jieqi.month_gan_zhi(dt)`；已接入 paipan.py、bazi-engine.py、bazi-verify.py、bazi-full-verify.py。
> 连带修复：① ephem UTC→北京时（起运偏8h）；② 起运「气」误当「节」（逆排起运岁数偏小，左左0.96→6.05岁）。
> 门禁：`python3 scripts/bazi-jieqi-regression.py`（8447项校验 vs lunar-python裁判，跑不过不许提交）。
> 教训：**规则写进文件≠被遵守**，只有物理门禁算数。
...
## ✅ 已修复：年柱立春前错误（2026-06-24发现·魏齐案例/杨昌玉级联 → 2026-09-10根治·engine/jieqi.py）

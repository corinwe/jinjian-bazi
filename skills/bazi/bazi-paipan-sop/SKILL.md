...
### Step 4.3B — 报告骨架先行（写内容前先搭骨架+强制项占位符）
...
### Step 7.0 — 铁律0：写入→审计→说OK（每次老板指问题的必走流程）

> **来源**：SOUL.md 铁律0 + HERMES.md 做事风格标准
> **口诀**：写下→审计→说OK，三步少一不算完；文件有不算有，审计过了才是真
...
常见审计项：
  □ 如涉及藏干十神/五行生克 → 先跑引擎验证十神关系再下笔
  □ 如涉及强制内容缺失（婚姻子女重点年份表）→ grep 报告确认存在
  □ 如涉及品牌名 → grep "九龙\|金鉴" 确认0匹配
  □ 如涉及规则写入文件 → grep 规则关键词在目标文件中确认
```

...
第1步 — 理法篇：做功分析
  ...
  执行清单（逐条不跳）：
    ...
    □ 判五路财富 ─ ①禄 ②库 ③官杀 ④财星 ⑤食伤 → 逐路原局+大运+流年
      ...
      逐路填写后再写结论
    □ **十神取自引擎** ─ 写每个十神前先`cat /tmp/{姓名}_engine.json | python3 -c "import json,sys;d=json.load(sys.stdin);[print(x['shi_shen']) for x in d.get('ge_ju',{}).get('shi_shen',[])]"`确认，禁止手写十神名
    □ **禁止引入不存在字** ─ 报告中每出现一个天干/地支，必须能在以下位置找到：
      原局四柱八字 / 大运序列 / 流年 → 找不到就不能写
    □ **藏干作用优先级** ─ 同柱藏干之间优先作用 > 不同柱藏干之间作用
      跨柱藏干作用需要大运/流年引动才能激活，原局中不自动发生
    □ **"用"包含食伤** ─ 盲派体用判定中，「用」= 财 + 官 + 食伤（人生追求的目标）
      亥水藏壬水伤官→属于"用"的范畴，必须列入体用判定

...
```
🚨 引擎数据完整性检查表（写报告前逐项打勾）：
  □ ENGINE['八字']存在 → 格式正确（4柱用空格分隔）
  □ ENGINE['四柱']存在 → 年柱/月柱/日柱/时柱
  □ ENGINE['日主']存在 → 与八字第3柱天干一致
  □ ENGINE['日主五行']存在 → 木火土金水
  □ ENGINE['身强弱']存在 → 总分 + 等级
  □ ENGINE['藏干']存在 → 年支/月支/日支/时支（每个至少1个藏干）
  □ ENGINE['十神']存在 → 年干/月干/日干/时干
  □ ENGINE['纳音']存在 → 年柱/月柱/日柱/时柱
  □ ENGINE['大运']存在 → 序列≥6步藏干类型优先
  ▶ 以上缺任何一项 → STOP → 先调排盘CLI补全 → 再继续

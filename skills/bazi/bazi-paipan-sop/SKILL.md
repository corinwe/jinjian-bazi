...
> 
> **🚨 数据层+模块层分离架构（2026-07-16 老板核心指令）**：
> 1. **唯一数据源** — 引擎JSON → `bazi-data-source.py` 验证+锁定 → `/tmp/{姓名}_datasource.json`（23字段）
> 2. **物理约束** — 写报告前设 `export BAZI_DATASOURCE=/tmp/{姓名}_datasource.json`，`precheck.py` 在pre_tool_call时检查此变量
> 3. **模块取数** — 每个报告模块的签名必须是 `def module(DS)`，从DS['xxx']取数，不从记忆取
> 4. **报告 ≠ 数据堆砌** — 报告是「DS数据 + 规则应用 = 分析结论」，不是把DS['xxx']字段打印一遍
> 5. **质量门禁** — 推库前跑 `verify-report-quality.py`，检查21§完整性、≥800行、数据源引用≥3处
> 
> **⚠️ 新增通用分析逻辑**（2026-07-22）：在格局判定后、写报告前，必须加载 `bazi-foundation-analysis` 的 §37（12项通用分析逻辑）和 §38（六体系协同分析框架）。含：从杀格判定三条件、家庭合参、创业合参、单动vs双动机制、置业规律、巳字应期落空提醒、分线断法（流年五条线分开判断）、合化真假甄别。详见 `references/实证反推_12项体系化修正_20260722.md`、`references/四份专业文档精读_体系化提升_20260722.md` 和 `references/Kimi3六套分析体系_吸收方案_20260722.md`。
...
### Step 4.1 — 🚨 强制加载对应事象技能 + 引擎数据完整性检查（不可跳过，写报告前必须执行）

> ⚠️ **第一步必须是引擎加载** — 不是读技能，是先确认ENGINE存在且完整。
> **引擎命名规则**：`/tmp/{姓名}_{时辰}_engine.json`（单时辰免去`_时辰`）
> **降级方案**：引擎不存在→调`python3 bazi-engine.py {出生年} {月} {日} {时} {分} {性别} {姓名} --json > {路径}`
...
#### ⑨ 重点流年年份 + 分线吉凶总表（完整版报告必含·2026-07-27新增）

> **来源**：2026-07-27三造多流派报告实践 + 老板校准
> **判断**：完整版深度报告必含，精简版可省略

**内容要求：**
1. **重点流年年份表** — 已发生关键流年，逐条子平+盲派+九龙三派合参
2. **未来发展全景** — 未来3-5年逐流年分析，三流派角度各自判断
3. **未来发展方向** — 事业/财富/子女/健康四大方向
4. **关键时间事件·流月应期** — 每年的重点月份和应事节点
5. **流年吉凶总表·分线断法** — 五线分断表，每线明确【吉】【凶】【平】
...
|:----|:-----|:----|:----|:----|:------|:-----|
```
每条线必须明确标注【吉】【凶】【平】，不能模棱两可。详见 skill_view('bazi-foundation-analysis') §38 分线断法

#### ⑩ 三决断流程（第一步必做·2026-07-27新增）

> **来源**：heluwenxian.nat100.top 产品设计 + 老板校准
> **原则**：先验过去，再看当下选择。

```yaml
三决断执行流程：
  Step 1 — 排盘后，给出3条可验证的过去判断
  Step 2 — 覆盖三个不同人生阶段或领域
  Step 3 — 每条标注"条1/条2/条3，您可以对照看看"
  Step 4 — 用户确认后 → 进入深度分析
  
多流派三决断（可选）：
  同时输出子平/盲派/九龙三个流派的简版三决断
  用户反馈准的流派 → 以该流派为主体做深度分析
```
...
### 🚨 大运真假判定精确规则（2026-07-16 老板校准·申中戊壬遗漏触发）

> **来源**：家主八字庚申癸未辛亥辛卯，年支申藏戊(余气30%)和壬(中气60%)。大运戊子时，戊土在原局申中有根→应为真运，报告误判为假。老板指出后，确立精确规则。

```yaml
大运真假判定（泉师兄三法·精确版）：

规则① — 原局有 = 真。原局没有 = 假。
  □ 天干：在天干透出 或 在地支藏干（本气/中气/余气）存在 → ✅ 真
  □ 地支：在四柱地支中出现 → ✅ 真

⚠️ 致命陷阱 — 藏干余气也算"有"
  ❌ 错误：认为戊土只在干支透出才算有 → 申中戊(30%余气)被忽略 → 大运戊子判假
  ✅ 正确：申中戊(余气30%)存在 → 戊在大运有根 → 天干戊为真
  ✅ 规则细化：任何藏干（本气/中气/余气）只要有对应的天干，该天干在大运出现即为真
  ✅ 规则细化：任何藏干（本气/中气/余气）只要有对应的天干，该天干在大运出现即为真

实战检查清单（每次写大运真的假前执行）：
  □ 大运天干X：查原局四柱天干有无X？查四柱地支藏干（本气/中气/余气）有无X？
  □ 大运地支Y：查原局四柱地支中有无Y？
  □ 藏干查法：年支×月支×日支×时支 → 列出所有藏干的五行/十神 → 再比对大运

常见错误模式：
  ❌ 申中戊(余气30%)查漏 → 大运戊判假
  ❌ 亥中甲(中气60%)查漏 → 大运甲判假
  ❌ 未中丁(中气60%)查漏 → 大运丁判假
  ✅ 正确：三库（辰戌丑未）的余气必须穷举到，申亥寅巳的中气必须穷举到
```
...
第3步 — 体用（八字方向）
  ...
  执行清单：
    ...
    □ **"藏干财官算用"** ─ 盲派「用=财+官」不分天干藏干。天干财官、地支藏干财官全部算用。禁止发明「天干才算用藏干不算」等排除规则。
...
第5步 — 做功（八字的具体运作）
  ...
  skill: bazi-foundation-analysis §3A.9-§3A.10 + bazi-wealth-analysis 五路财富检查表
  前置条件：必须先走完第1-4步，不可跳步直接做功
  执行清单：
    □ 判做功结构 ─ 制用/化用/生用/合用/墓用五种
    □ 合用结构专项 ─ 日主合财/官？日支六合财/官？三合局化财/官？拱合出财/官？
    □ 判生用结构 ─ 食伤生财路线是否清晰？双食伤？单食伤？
    □ 判功神废神 ─ 参与做功=功神，不参与=废神
    □ 判做功效率 ─ 合制>冲制>克制
    □ 口诀条件校验 ─ 引用任何口诀/断语前，先逐条核对条件是否满足本八字
      常见陷阱：四柱皆带财(查年柱有无财)、身弱不担财(核实身弱与否)
      必查：泉师兄口诀/断语引用前，条件不满足→不准引用，须注明差异
    □ 判家里家外 ─ 财官归属权（主位/宾位）

    ▸ **盲派报告必含三项（2026-07-16老板校准·所有盲派报告强制）**：
    □ 🚨 家里家外做功效率+财官量级分析（插入做功§后）
       格式：家里(X→Y)效率:高/中/低 量级:X级 家外(→)效率:X 量级:X 比例:家里X%家外Y%
       必查：§3A.11核查清单5项（谁做功/效率/家里量级/家外量级/最终归宿）
    □ 🚨 财官库分析（插入财富§附近）
       格式：财库在X(日/月/年),状态:冲开/墓/未冲,大运开库窗口:X运
       必查：§3A.12四步法（查财库→查状态→查官库→大运窗口）
    □ 🚨 一生做功变化全景（插入大运§后）
       格式：8运主力做功→换功节点→爆发期→最低谷→最终归宿
       必查：§3A.13核查清单5项
...
### 🆕 Step 4.3B — 🚨 Harness Engine 规则驱动模式（2026-07-16新增·本session）

> **🛡️ 本session致命教训（已固化）**：
> - 文昌贵人公式：用**年干**而非日干（庚→亥✅，非辛→子❌）。见 `bazi-data-source.py` calc_shensha()
> - 财星查找：必须同时查**天干十神**和**藏干十神**。年干辛也可能=正财！
> - 多份报告：禁止复制模板改名字 → 每份从各自数据源独立生成
> - 数据源字段名：`DS['年干']` 不用 `B8[0]`（索引方式容易搞混）

> **关键区分**：旧模式（gen_report_v4.py）将规则硬编码在Python的if-else中 → 改规则必须改代码 → 每次改代码都可能引入新Bug。
> **新模式**：规则独立为YAML文件 + 模板独立为.md文件 + 引擎只做规则匹配和数据翻译。

```
目录结构：
  skills/bazi/harness-engine/
    ├── workflow/            # workflow_v2.yaml — Phase定义
    ├── rules/               # 每§独立规则文件
    │   ├── cai_fu.yaml      # 财富分析规则（含开财库）
    │   └── shi_ye.yaml      # 事业分析规则
    ├── templates/           # 输出模板（{变量}由引擎填充）
    ├── engine/              # 确定性代码
    │   ├── workflow_v2.py   # 工作流定义
    │   └── step_runner.py   # 步骤执行器
    └── test_suite/          # 评估集（L3飞轮）

每§生成流程：
  1. 从DS读取该§需要的字段（DS['年干']等，非数组索引）
  2. 加载 rules/对应.yaml（规则独立文件，不硬编码）
  3. 加载 templates/对应.md（输出格式模板）
  4. 引擎匹配规则 → 填充模板变量
  5. Sensor验证 → 通过则下一步，不通过则L1修正

详见 references/harness-loop-architecture-20260716.md
```

### 🆕 Step 4.3C — 🚨 数据源字段命名约定（2026-07-16新增·本session致命教训）

> **致命教训**：本session中，代码用 `DS['八字段']`（汉字八）但数据源实际存为 `DS['8字段']`（数字8）→ KeyError。
> 同一session中，`G = [B8[0], B8[2], B8[4], B8[6]]` 的索引错误导致月令取成了时干（"壬"而非"未"）。

```
字段命名铁律：
  ✅ 天干用：DS['年干'], DS['月干'], DS['日干'], DS['时干']
  ✅ 地支用：DS['年支'], DS['月支'], DS['日支'], DS['时支']
  ❌ 禁止用：DS['8字段'][0] 或任何数组索引方式
  ✅ 四柱用：DS['四柱']['年柱'], DS['四柱']['日柱']
  
  原因：
    1. 数组索引容易搞混顺序
    2. 字段名"年干""月支"语义明确，不会歧义
    3. 数据源已独立存储这些字段，无需从8字段数组取
...
### 🆕 Step 4.3D — 🚨 十神阴阳规则已修正（2026-07-16·本session）

> **Bug根因**：`bazi-data-source.py`中calc_shishen()函数的同阴阳/异阴阳规则倒置。
> **实例**：子源年支乙木对丙火日主→阴生阳=异阴阳=正印。旧代码返回了"偏印"（老板指出后才修正）。

```
规则修正（2026-07-16 session）：
  正印/偏印: 同阴阳=偏印, 异阴阳=正印（原代码：异阴阳→偏印 ❌）
  七杀/正官: 同阴阳=七杀, 异阴阳=正官（原代码：异阴阳→七杀 ❌）
  正财/偏财: 同阴阳=偏财, 异阴阳=正财 ✅（无需改）
  食神/伤官: 同阴阳=食神, 异阴阳=伤官 ✅（无需改）
  比肩/劫财: 同阴阳=比肩, 异阴阳=劫财 ✅（无需改）
...
IF 出生时辰不确定（有两个或以上可能时辰）
  ...
  第3步 — 派发前context质量检查（🚨 新增·2026-07-16）
    □ 体用定义是否正确？（体=日主+印+比劫+食伤，用=财+官，藏干财官一律算用）
    □ 有无「不算」「排除」「不纳入」等排除规则？如有→删除
    □ 算术自检：逐项加和是否等于总分？
    □ 口诀/断语条件是否满足本八字？
    □ context中引用的技能版本是否为最新？（skill_view('bazi-foundation-analysis')确认§3A.9铁律在）
    → 以上5项全部通过才可派发，缺一不可
    → 口诀：派发之前五步查，体用算术别漏下；排除规则不能有，条件校验不能假
...
### Step 5.1 — 🚨 引擎数据 vs 报告内容自动对齐（2026-07-16新增）

> **根因**：子源引擎说身强弱=中和(55.6分)，子代理报告写身强，老板指出后才发现数据不对齐。
> **铁律**：报告中每个数字必须能在ENGINE中找到直接来源。

```bash
# 🚨 自动对齐检查清单（报告写完提交前执行）
□ 报告中「身强弱得分」= ENGINE['身强弱']['总分']？
□ 报告中「身强弱等级」= ENGINE['身强弱']['等级']？
□ 报告中「大运起始/终止年龄」= ENGINE['大运']['序列'][i]['起始年龄']/['终止年龄']？
□ 报告中「日主」= ENGINE['日主']？
□ 报告中引用的藏干ENGINE['藏干']中存在？
→ 以上5项全部通过→可提交。任一不符→修正报告后重新校验。
```
...
**执行顺序**: 先跑5.1(~5.4)，再跑5.5门禁+5.6质量门禁。全部通过才进入Phase 6推库。

### Step 5.6 — 🚨 L3回归测试（2026-07-16新增·本session建设）

> **L3飞轮：每次修改引擎/规则/模板后必须跑回归测试，验证已有能力不退化。**
> **驱动：老板每指出一个错误 → 加一条测试用例 → 永久不再发生。**

```bash
cd skills/bazi/harness-engine
python3 test_suite/regression.py
# → exit 0 = 全部通过
# → exit 1 = 有测试失败 → 修复后重新运行
```

**当前测试用例**（2026-07-16）：
- `test_zhu_ren.json` — 魏启令（日主辛金、身强64分、八字正确）
- `test_ziyuan.json` — 子源（日主丙火、八字正确）

**添加新测试用例流程**：
1. 修改规则/引擎后，运行回归测试确认基线通过
2. 老板指出新错误 → 写一条测试验证
3. 测试文件放入 `test_suite/` 目录
4. 回归测试 ALL GREEN

### Step 5.7 — 🚨 21§报告质量物理门禁（2026-07-16新增·亲自验证）
> **推库前最后一道门禁。必须 exit 0 才可进入Phase 6。**
> **主母复制Bug（2026-07-16）**：曾将魏启令报告作为模板仅替换名字生成了主母/子源报告，导致数据完全错误。
> **铁律**：每份报告用各自数据源独立生成，禁止复制模板改名字。
...
```
🔴 错误方式（复制模板改名字）：
  cp 魏启令报告.md 主母成报告.md    # ← 只改了文件名，数据全是魏启令的
  sed 's/魏启令/主母成/g'          # ← 只替换了名字字符串
...
### Step 6.1 — 放入人物档案（禁止写入/root/ 禁止写入00-原始素材）

> 🚨 **2026-07-27 老板校准**：人物报告必须放入 `02-人物档案/编号-姓名/`，禁止放入 `00-原始素材/`。
> **00-原始素材/** 只存外部导入的原始资料（Kimi文档、公众号文章等），不是人物报告的位置。
> **人物报告**归人物档案，详细规则见 `references/file-organization-rules.md`。
> **常见错误**：将一家三口报告放入 00-原始素材/ → 老板指出后更正。
> **核查流程：归档前检查路径是否含"02-人物档案"且不含"00-原始素材"。**

...
| 文件 | 角色 |
|:----|:-----|
| `/root/bazi-platform/engine/_gen_detail_analysis.py` | 核心模块（24,000字符）21个§的生成函数 |
| `/root/bazi-platform/engine/generate_deep_report.py` v3.0 | 深度报告生成器，消费detail_analysis |
...
- 引擎入口：`/root/bazi-platform/engine/pipeline_product.py --json`
...
tools:
  ...
  filesystem:
    allowed_paths:               # 文件工具只能访问这些路径
      ...
      - "/root/bazi-platform/skills"    # 9个bazi技能
...
> **致命场景（2026-06-26）**：我写了 `/root/bazi-platform/engine/paipan.py` 作为平台的前端排盘引擎，结果月柱+日柱全部算错。老板指出：「你之前不是写好了一个引擎吗？直接用不行了吗？」
>
...
>    ——物理防御已创建: `/root/bazi-platform/scripts/bazi-must-run-engine.sh`
...
```yaml
修改引擎代码后必须清除缓存：
  find /root/bazi-platform -name '__pycache__' -exec rm -rf {} + 2>/dev/null
...
```yaml
❌ /root/bazi-platform/engine/paipan.py — 已废弃，不再使用
...
引擎JSON的起运年份计算规则（Q4进位）：
  起运日期 = 出生日期 + 起运年龄(天)
  实用起运年份 = 起运日期的年份
  起运在10~12月(Q4) → 进位到次年
  例：1980-08-06 + 0.38年 → 1980-12-19(12月Q4) → 1981年起运 ✅
  例：2011-05-31 + 8.52年 → 2019-11(11月Q4) → 2020年起运 ✅
  例：1987-07-10 + 9.55年 → 1996-12(12月Q4) → 1997年起运 ✅

子agent的context中必须写明：
  「大运年份已从引擎JSON提取，直接使用 data['大运']['序列'] 的每步年份数据，
   不要自行计算。报告中所有大运年份必须与引擎JSON完全一致。」
...
**文件位置**：`/root/bazi-platform/engine/` — 36个Python文件，合计 **12,437行**确定性代码。
...
**运行方式**：`cd /root/bazi-platform/engine && python3 tests/test_full_suite.py`
...
> **2026-06-26更新**：原型已扩建为全栈平台（见v7.0）。引擎目录移至 `/root/bazi-platform/engine/`，增加后端+前端。
...
位于 `/root/bazi-platform/engine/` 的 v4.0 全量引擎：
...
API接受JSON（含name/gender/birth_year/birth_month/birth_day/birth_hour/calendar），返回21§结构化输出。前端SPA位于 `/root/bazi-platform/frontend/index.html`，含完整报告预览+PDF下载。
...
`/root/bazi-platform/engine/comprehensive.py` 中的函数（analyze_appearance / analyze_property / analyze_career_detail / analyze_children / analyze_health_detail / generate_three_verdicts / generate_da_yun_curve / generate_wu_xing_advice / generate_life_advice）全部为**硬编码占位符**：
- 不依赖任何skill文件的规则
- 不检查八字具体藏干的实际情况
- 不对比客户八字与规则条件的匹配
...
> **发现**：`/root/bazi-platform/frontend/` 下存在两个独立的SPA实现：
...
```yaml
引擎模块清单（全部位于 /root/bazi-platform/engine/）：

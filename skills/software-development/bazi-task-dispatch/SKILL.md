...
## 📋 老板沟通规范（Sub-Agent上下文强制携带）

> **说明**：金鉴真人（老板魏启令）的沟通风格和底线原则。每次派Sub-Agent时，在context末尾附带此规范。

### 沟通风格

| 老板说的 | 实际意思 | Sub-Agent回应方式 |
|:---------|:---------|:------------------|
| '那继续' / '看一下' | 直接行动，不要解释 | 直接干活，不废话 |
| '跑一下看看' | 给结果，不给空话 | 跑完给数据，不解释理论 |
...
| 原则 | 含义 |
|:-----|:------|
| **引擎定量 > LLM定性** | 所有命理推理必须从引擎取数据，不自行计算 |
| **点面体系验证** | 修1个Bug→延伸同逻辑其他文件→延伸整个体系 |
| **内容级验证** | 每个输出展示三段式证据链：数据来源→规则来源→结论 |
| **零自创断事逻辑** | 所有断语必须有原始理论行号/原文支撑 |
| **体系化修复>表面patch** | 修完一个Bug必须查：引用该模块的所有文件是否同步更新 |
| **都修掉再继续** | 一个Phase没做完不进入下一个Phase |
| **修复引擎≠提升报告质量** | 数据修对了不等于报告内容合格，报告质量靠亲自逐句写 |

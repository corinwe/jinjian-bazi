...
delegate_task(
    ...
    - 引擎路径：/root/bazi-platform/engine/
    - 测试命令：cd engine && python3 tests/test_full_suite.py
    - Git：cd /root/bazi-platform && git add -A && git commit -m "msg" && git push
    """,
...
delegate_task(
    ...
    context=f"""
    🎯 **目标：**
    在 /root/bazi-platform/engine/ 下实现 [模块名] 模块
    ...
    🛑 **终止条件：**
    Success = cd /root/bazi-platform/engine && python3 tests/test_full_suite.py — 全部PASS
    
    🏗️ **项目上下文：**
    - 引擎路径：/root/bazi-platform/engine/
    - 运行测试：cd /root/bazi-platform/engine && python3 tests/test_full_suite.py
    - 模块导入验证：cd /root/bazi-platform/engine && python3 tests/test_imports.py
    """,
    toolsets=['terminal', 'file']
)
```
...
delegate_task(
    ...
    📋 **格式：**
    - 前端文件：/root/bazi-platform/frontend/
    ...
    🛑 **终止条件：**
    Success = python3 /root/bazi-platform/engine/tests/test_e2e.py --remote — 16/16通过
    """,
...
delegate_task(
    ...
    📋 **格式：**
    - 测试文件：/root/bazi-platform/engine/tests/test_[module].py
...
delegate_task(
    ...
    📋 **格式：**
    输出21§完整报告，格式参考 /root/bazi-platform/engine/tests/test_full_suite.py 中的§结构
    ...
    🛑 **终止条件：**
    Success = cd /root/bazi-platform/engine && python3 tests/validate_all.py — 全部PASS
    """,
    toolsets=['terminal', 'file']
)
```
...
delegate_task(
    ...
    📋 **格式：**
    输出markdown格式报告，符合 /root/bazi-platform/skills/bazi-report-template/SKILL.md
...
**案例：** 2026-06-30，career-analysis 被写入 `/root/weiwuji-knowledge-base/.../skills/bazi/` 而非 `/root/bazi-platform/skills/bazi/`。

...
**路径**: `projects/bazi-platform/scripts/bazi-must-run-engine.sh`
**创建时间**: 2026-06-29
**触发教训**: 梦的日柱手算错误（壬戌→癸亥）
...
如果不知道时辰，不加 -h 参数：
```bash
bash projects/bazi-platform/scripts/bazi-must-run-engine.sh -n 梦 -g 女 -y 2007 -m 7 -d 27
```
输出12个时辰的排盘结果。
...
| 文件 | 作用 |
|:-----|:------|
| `projects/bazi-platform/AGENTS.md` | 铁律①自动加载 |
| `projects/bazi-platform/.hermes/config/credentials.md` | 铁律完整版 |

...
- PAT: 存于 projects/bazi-platform/.hermes/config/credentials.md
- 推workflow需 `workflow` scope（当前PAT只有repo scope）
- PAT值获取: 读 credentials.md 或问老板

## 部署
- 路径: projects/bazi-platform/
- API: http://43.162.90.39:8000/
- 文档: http://43.162.90.39:8000/docs
- 启动: cd /root/.hermes/profiles/jinjian-zhenren/projects/bazi-platform && python3 -m uvicorn api.main:app --host 0.0.0.0 --port 8000
...
## 核心路径
- 引擎: projects/bazi-platform/engine/
- 排盘门禁: projects/bazi-platform/scripts/bazi-must-run-engine.sh
- 测试: projects/bazi-platform/engine/tests/
- 验证: cd projects/bazi-platform/engine/tests && python3 validate_all.py
- 前端: projects/bazi-platform/frontend/
...
- bazi-must-run-engine.sh 执行时自动:
  ...
  2. 调用 `python3 projects/bazi-platform/scripts/canggan-parse.py /tmp/bazi_last_result.json`
  3. 输出四柱每个地支的藏干、主气十神
  4. 标注「易混淆」项（辛+午=七杀⚠️）
- 来源: 2026-07-06 分析老板最佳时柱时午火=七杀误判为正官

### 铁律⑳ — 四柱分析结论5关校验（2026-07-06 新增）
- 命令: `python3 projects/bazi-platform/scripts/pillar-verify.py`
- 5关内容:
  1. 五鼠遁校验：候选时柱天干真实存在
  2. 藏干十神校验：易混淆项强制标注
  3. 结构优先级校验：三合局>单柱双喜
...
| **四柱校验(5关)** | **`python3 projects/bazi-platform/scripts/pillar-verify.py`** |
| 部署前检查 | 见HERMES.md |

...
## 📐 SOUL.md 瘦身铁律（2026-07-16 企业级架构新增）

> **核心原则**：SOUL.md 只放「身份+铁律」，所有操作规则放到 HERMES.md 或 .hermes.md。

### 为什么？

```yaml
SOUL.md 是系统级（永远加载，每次会话自动注入）：
  → 适合放：你是谁、你的铁律、绝对不能违反的原则
  → 不适合放：操作步骤、SOP流程、技能引用列表

HERMES.md / .hermes.md 是项目级（or链，按CWD加载）：
  → 适合放：项目级SOP、Sub-Agent分工、任务派发流程、反模式清单
```
...
```
SOURCE: SOUL.md          →  TARGET: HERMES.md
身份声明 + 铁律           ✅ 保留
Sub-Agent分工体系         ❌ 迁移
任务派发流程              ❌ 迁移
端到端流程                ❌ 迁移
...
```yaml
❌ SOUL.md 写了300+行 → 80%是操作规则不是身份→瘦身
❌ SOUL.md 里写了具体文件路径 → 路径会变
❌ SOUL.md 里写了技能引用列表 → 应由技能本身管理
❌ 以为越长记得越牢 → 越长重点越淹没
```

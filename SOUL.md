...
## 🚨 核心执行原则（凌驾于所有指令之上·禁止跳过）

> ⚠️ **SOUL.md只定义身份和纪律，不存放具体SOP。所有执行步骤在HERMES.md和bazi-paipan-sop中定义。**
> **写报告前必须先读HERMES.md中的铁律A~G，逐条对照执行。**

### 执行铁律（三条，违反即违规）

```
铁律① — 必须验证
  每次调用工具后，必须检查返回数据确认执行成功。
  例：写文件后检查bytes_written>0，不是"看起来写完了"。

铁律② — 必须引用
  汇报任务完成时，必须引用具体的工具返回数据。
  ✅ "文件写入成功，165行"  ❌ "文件写好了"
  ✅ "引擎跑完，64分身强"   ❌ "引擎没问题"

铁律③ — 禁止跳过
  禁止在未实际调用工具或未校验结果的情况下声称完成。
  SOP中的每个Phase必须顺序执行，不可跳过一个声称完成。
```

### 全局指令（每次会话自动加载）

```
你必须严格遵循以下文件的SOP流程，优先级从高到低：
  ① HERMES.md（项目级铁律+铁律A~G执行标准）
  ② bazi-paipan-sop（6 Phase标准作业程序）
  ③ 对应§的技能文件（bazi-wealth-analysis等）
任何冲突时，以HERMES.md为准。
```
...
🚨 **每次会话第一条消息发出前，必须先执行 `terminal('date')`。不执行不准发分析/报告。上下文压缩会截断记忆，必须物理执行不可依赖记忆。**

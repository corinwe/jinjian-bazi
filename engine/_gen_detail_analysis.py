...
def _shen_qiang_ruo_detail(result: dict) -> str:
    ...
    parts.append("【金鉴真人·身强弱规则·月令本气印=40分·比劫全算】")
    parts.append(
        f"月令印星计{det.get('yue_yin', 0)}分 | 月令比劫计{det.get('yue_bi', 0)}分 | "
...
def _wealth_detail(result: dict) -> str:
    ...
    parts.append("【金鉴真人·财星规则·只含正偏财·不含劫财】")
    parts.append(
        f"财星明细：月令{cd.get('yue', 0)}分 | 日支{cd.get('ri', 0)}分 | "
...
def _education_detail(result: dict) -> str:
    ...
    # 学校六档标准速查
    parts.append(
        "【学校等级六档标准】 👑顶尖(≥5项✅+文昌月令) > 🥇985(3-4项✅+文昌日/月) > 🥇211(3项✅+文昌在局) > 🥈普通(2项✅+文昌大运) > 🥉大专(1-2项✅+文昌缺) > 🪜初中(≤1项✅+无印无文昌)"
    )
    parts.append("【金鉴真人·学业规则·第0层三档法+六步排查+学校六档+文昌双轨】")
    parts.append(
        "【规则引用】年柱有印→学业基因(素材12行517·素材03行541)；文昌=学业助力(文昌贵人理论)；印运时间线定学历层级"
    )
    return "\n".join(parts)
...
def _marriage_detail(result: dict) -> str:
    ...
    parts.append(
        "【规则引用】男命正财=妻星·女命正官=夫星(素材12)；夫妻宫十神断婚姻质量(婚姻skill)；四大信号排序(素材17)"
    )
    return "\n".join(parts)
    ...
    if has_wxot:
        ...
        for item in wxot:
            if isinstance(item, dict):
                parts.append(
                    f"  {_s(item, 'wx', '')}五行过{_s(item, 'count', '')}→{_s(item, 'organ', '')}。【金鉴真人·健康规则·五行过三】"
                )
    else:
...
def _da_yun_detail(result: dict) -> str:
    ...
    if isinstance(dy_list, list):
        ...
        for dy in dy_list:
            ...
            parts.append(
                f"  {star} {dy.get('gan_zhi', '')} {dy.get('start_age', '')}~{dy.get('end_age', '')}岁 "
        ...
        if best >= 0 and best < len(dy_list):
            ...
            parts.append("【金鉴真人·大运规则·纯喜用运=最佳·以喜用神逻辑排序】")
...
def _ge_ju_detail(result: dict) -> str:
    ...
    if isinstance(ss_list, list):
        ...
        for item in ss_list:
            if isinstance(item, dict):
                parts.append(
                    f"  {_s(item, 'position', '')} {_s(item, 'gan', '')}→{_s(item, 'shi_shen', '')}（{_s(item, 'yin_yang', '')}·{_s(item, 'wu_xing', '')}）"
                )
...
def _sec_1_detail(result: dict) -> str:
    ...
    parts.append(
        f"【日主】{_s(_s({}, 'ri_zhu', s1.get('ri_zhu', {})), 'gan', '')}（{_s(_s({}, 'ri_zhu', s1.get('ri_zhu', {})), 'wx', '')}）"
    )

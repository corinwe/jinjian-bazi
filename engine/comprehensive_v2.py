...
from constants import (
    ...
    WU_XING_SHENG, WU_XING_KE, CANG_GAN_RATIO, BASE_SCORE,
    BaZi, Pillar,
)
from shi_shen import get_shi_shen_for_gan, get_shi_shen_for_cang_gan, get_shi_shen_all_dry
...
def analyze_appearance(ri_zhu: str, shen_label: str, shen_score: float,
...
def analyze_property(ri_zhu: str, ri_zhi: str, xi_yong: list[str],
                      da_yun_list: list[dict], best_idx: int,
                      cai_xing_total: float) -> dict:
    """§9 置业分析"""
...
def analyze_career(ri_zhu: str, bazi_gans: list[str], bazi_zhis: list[str],
                    shen_label: str, shen_score: float,
                    xi_yong: list[str], ji_shen: list[str],
                    ge_ju_main: str, ge_ju_detail: str) -> dict:
...
SHI_ER_KOU_JUE = {
    "长生": "长生四子中旬半", "沐浴": "沐浴一双保安康",
    "冠带": "冠带临官三子位", "临官": "冠带临官三子位",
    "帝旺": "旺中五子自成行", "衰": "衰中二子病中一",
    "病": "衰中二子病中一", "死": "死中晚景少儿郎",
    "墓": "入墓之时多不良", "绝": "受气为绝一个子",
    "胎": "胎中头胎是姑娘", "养": "养中三子只留一",
}
...
def analyze_children(ri_zhu: str, gender: str,
                      hour_gan: str, hour_zhi: str,
                      bazi_gans: list[str], bazi_zhis: list[str],
                      da_yun_list: list[dict]) -> dict:
...
def analyze_health(bazi_gans: list[str], bazi_zhis: list[str],
                    ri_zhu: str, shen_label: str, shen_score: float,
                    xi_yong: list[str], liu_nian_events: list = None) -> dict:
    ...
    for i, g in enumerate(bazi_gans):
        ...
        if ss == "七杀":
            ...
            zha = YI_ZHA.get(f"year_gan" if i == 0 else f"month_gan" if i == 1 else "day_zhi" if i == 2 else "hour_gan", "")
            sha_risks.append({
                "qi_sha_position": position_name,
                "body_area": zha,
                "level": "实际病痛",
            })
    ...
    for i, z in enumerate(bazi_zhis):
        for cg, ratio in DI_ZHI_CANG_GAN.get(z, []):
            ...
            if ss == "七杀":
                ...
                zha = YI_ZHA.get(f"year_zhi" if i == 0 else f"month_zhi" if i == 1 else "day_zhi" if i == 2 else "hour_zhi", "")
                level = "本气→病已成型" if ratio >= 100 else "中气→潜在病灶" if ratio >= 60 else "余气→轻微不适"
                sha_risks.append({
                    "qi_sha_position": f"{position_name}(藏干{cg})",
                    "body_area": zha,
                    "level": level,
                })
    ...
    for i, g in enumerate(bazi_gans):
        ...
        if ss == "偏印":
            ...
            pian_yin_risks.append({
                "pian_yin_position": position_name,
                "side": side,
                "risk": f"{position_name}偏印→经络淤堵({side})",
            })
    ...
    return {
        ...
        "focus_areas": list(set([o["organ"] for o in over_three] + 
                                [r["body_area"] for r in sha_risks[:1]])),
    }
...
def generate_three_verdicts(shen_label: str, cai_score: float,
                             ge_ju_detail: str, best_da_yun: dict,
                             marriage_info: dict, career_grade_score: int) -> list:
    """§18 三决断 — 基于实际数据的确定性输出"""
    ...
    verdicts.append({
        ...
        "time": f"最佳运：{best_da_yun.get('gan_zhi','')}（{best_da_yun.get('start_age','?')}~{best_da_yun.get('end_age','?')}岁）" if best_da_yun else "大运配合",
...
DIET_MAP = {"金": "润肺食物（百合/银耳/豆腐）", "水": "补肾食物（黑豆/黑芝麻/海参）",
            "木": "养肝食物（菠菜/芹菜/绿豆）", "火": "养心食物（红枣/红豆/番茄）",
            "土": "养脾食物（小米/南瓜/黄豆）"}
...
def generate_life_advice(shen_label: str, cai_score: float,
                          xi_yong: list[str], ge_ju_detail: str,
                          da_yun_list: list[dict], marriage_info: dict,
                          career_grade_score: int) -> dict:
    """§21 人生建议"""
    xi = xi_yong[0] if xi_yong else "土"
    best_dy = ""
    if da_yun_list:
    ...
    return {
        ...
        "health": {
            "advice": f"体质{'偏强注意劳逸结合' if shen_label=='身强' else '偏弱注意调养'}",
        },
        "marriage": {
            "advice": f"婚姻质量{marriage_info.get('quality','中等') if marriage_info else '中等'}，注意沟通经营",
        },
        "social": {
            "advice": f"喜用{xi}→多与{xi}五行属性的人合作",
        },
    }
...
def run_comprehensive_engine(bazi: BaZi, 
                               shen_score: float, shen_label: str, shen_detail,
                               cai_detail,
                               ge_ju_main: str, ge_ju_detail: str,
                               xi_yong: list[str], ji_shen: list[str],
                               da_yun_list: list[dict], da_yun_scores: list,
    ...
    career = analyze_career_advanced(ri_zhu, all_gans, all_zhis, shen_label, shen_score,
                                      xi_yong, ji_shen, ge_ju_main, ge_ju_detail)
    
    # 财富分析（五层动态体系）
    wealth = analyze_wealth_advanced(ri_zhu, all_gans, all_zhis, shen_label, shen_score,
                                      cai_detail.total, xi_yong, dy_list_with_score)
    
    # 外貌分析
    appearance = analyze_appearance(ri_zhu, shen_label, shen_score, all_gans, all_zhis, ge_ju_main)
    
    # 置业分析
    property_analysis = analyze_property(ri_zhu, bazi.day.zhi, xi_yong,
                                          dy_list_with_score, best_idx, cai_detail.total)
    
    # 子女分析（v2完整版 — 十二长生+出生年份推理+父母合参）
    children = analyze_children_advanced(ri_zhu, bazi.gender, bazi.hour.gan, bazi.hour.zhi,
                                          all_gans, all_zhis, dy_list_with_score)
    
    # 健康分析（v2完整版 — 五行过三+七杀断病法+流年预测）
    health = analyze_health_advanced(all_gans, all_zhis, ri_zhu, shen_label, shen_score, xi_yong,
                                      age=current_year - birth_year)
    
    # 三决断
    career_grade_score = career.get("grade_score", 5)
    verdicts = generate_three_verdicts(shen_label, cai_detail.total, ge_ju_detail,
                                        best_dy, marriage_result, career_grade_score)
    ...
    # 人生建议
    advice = generate_life_advice(shen_label, cai_detail.total, xi_yong, ge_ju_detail,
                                   dy_list_with_score, marriage_result, career_grade_score)

...
def _detect_talent_36(
    all_ss: list[str], bazi_gans: list[str], bazi_zhis: list[str], ri_zhu: str, shen_label: str, shen_score: float
) -> list[str]:
    """36命格职业类匹配"""
    talents = []
...
def analyze_career_full(
    ...
    for i, ss in enumerate(gans_ss):
        if ss == "正官":
            if shen_label == "身强":
                guan_sha_analysis.append("正官透干+身强→利管理晋升")
            else:
                guan_sha_analysis.append("正官透干+身弱→压力大")
        if ss == "七杀":
            if shen_label == "身强":
                guan_sha_analysis.append("七杀透干+身强→杀伐决断")
            else:
                guan_sha_analysis.append("七杀透干+身弱→压力/小人")
    ...
    if wei_ren_ge_name:
        ...
        detail_parts.append(
            f"【伟人格判定】{wei_ren_ge_name}——{wr_info.get('principle', '')}。{wr_info.get('comb', '')}。"
        )
    ...
    if shen_label == "身强":
        detail_parts.append(
            f"【身强定模式】身强{shen_score:.0f}分，有能力担事，适合管理/领导岗位。官杀为喜用时利晋升，越挫越勇，可挑战高压岗位。"
        )
    elif shen_label == "身弱":
        detail_parts.append(
            f"【身弱定模式】身弱{shen_score:.0f}分，有能力但容易累，适合技术/专业岗。印为喜用时需贵人/平台托底。杀印相生→以智慧化压力为成就。"
        )
    else:

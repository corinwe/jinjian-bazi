...
from constants import TIAN_GAN, DI_ZHI, TIAN_GAN_WU_XING, WU_XING_SHENG, WU_XING_KE
...
def analyze_education(
    ...
    """
    # ── Step 1: 印星情况 ──
    has_yin = is_tou_gan("", BaZi.__new__(BaZi), "正印") or is_tou_gan("", BaZi.__new__(BaZi), "偏印") if False else False
    # 简化: 检查天干是否有正印/偏印
    yin_score = 0
    yin_detail = ""
    for g in bazi_gans:
    ...
    # ── Step 6: 综合判定 ──
    checks = 0
    if yin_score >= 1: checks += 1
    if guan_sha_count <= 1: checks += 1
    if shi_shang_count >= 1: checks += 1
    if wen_chang_in_local or wen_chang_in_da_yun: checks += 1
    if da_yun_edu_score >= 1: checks += 1

...
ZHIZANG_BASE: dict[str, float] = {
    "年": 4.0, "月": 40.0, "日": 12.0, "时": 12.0
}
...
YIZHA_INDEX: dict[str, int] = {
    "年干": 0, "年支": 1, "月干": 2, "月令": 3, "日支": 4, "时干": 5, "时支": 6,
}
...
def count_wuxing_numeric(gans: list[str], zhis: list[str], include_rizhu: bool = True) -> dict[str, int]:
    ...
    # 地支本气
    for zhi in zhis:
        if zhi in ZANGGAN_TABLE and ZANGGAN_TABLE[zhi]:
        ...
        if zhi in ZANGGAN_TABLE:
            for cang_gan, ratio, level in ZANGGAN_TABLE[zhi]:
                if cang_gan == qisha_gan:
                    ...
                    severity_map = {"本气": "病已成型，症状明显", "中气": "潜在病灶，时有发作", "余气": "轻微症状，偶有不适"}
                    results.append({
                        "类型": "七杀（实疾）",
                        "位置": pos_name,
                        "地支": zhi,
...
def _get_chong_disease(chong_name: str, level: str) -> str:
    ...
    return tg_map.get(chong_name, f"未具体分类，需根据五行生克分析")
...
def check_mu_special(bazi_gans: list[str], bazi_zhis: list[str], scores: dict[str, float]) -> dict[str, Any]:
    ...
    if nian_wx_gan in ("金", "土") or nian_wx_zhi in ("金", "土"):
        result["发质判断"] = f"年柱落{egan if (egan:=(nian_wx_gan or nian_wx_zhi)) else ''} → 发质不好（{'金克木' if nian_wx_gan=='金' or nian_wx_zhi=='金' else '土囚木'}）"
...
def check_wuxing_cycle(scores: dict[str, float]) -> dict[str, Any]:
    ...
    if len(missing) == 0:
        ...
        result["流通状态"] = f"⚠️ 缺{'/'.join(missing)} → 循环中断 → {', '.join([f'{wx}对应的器官弱' for wx in missing])}"
        if "水" in missing:
    ...
    for w1, w2 in pairs:
        if scores.get(w1, 0) > 50 and scores.get(w2, 0) > 50:
            ...
            if key in LIANGWANG_DISEASE:
                results.append(f"{w1}{w2}两旺（{w1}={scores[w1]:.0f}分, {w2}={scores[w2]:.0f}分）→ {LIANGWANG_DISEASE[key]}")
    return results
...
def analyze_health_full(
    ...
    severity = assess_severity(
        wuxing_over_three, qisha_results, liuchong_results, gongwei_injury
    )

    # 五行调和法建议
    deficient_wuxing = [wx for wx in ["木", "火", "土", "金", "水"]
                        if wuxing_scores.get(wx, 0) < 10]
...
def quick_health_score(bazi_gans: list[str], bazi_zhis: list[str], age: int = 35) -> dict[str, Any]:
    ...
    return {
        ...
        "问题数量": issues,
        "问题明细": details,
        "评级": "优" if adjusted_score >= 80 else "良" if adjusted_score >= 60 else "中" if adjusted_score >= 40 else "差",
    }

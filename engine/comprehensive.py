...
from constants import TIAN_GAN_WU_XING, DI_ZHI_WU_XING, DI_ZHI_CANG_GAN, WU_XING_SHENG

# ── §7 身材外貌 ──
def analyze_appearance(ri_zhu: str, shen_label: str, shen_score: float,
                        bazi_gans: list, bazi_zhis: list) -> dict:
...
# ── §9 置业/买房 ──
def analyze_property(ri_zhu: str, ri_zhi: str, xi_yong: list,
                      da_yun_list: list, best_idx: int) -> dict:
...
# ── §10 事业详析 ──
def analyze_career_detail(ri_zhu: str, bazi_gans: list, shen_label: str,
                           xi_yong: list, ge_ju_detail: str) -> dict:
    wx = TIAN_GAN_WU_XING[ri_zhu]
    career_map = {
        "木": "教育/文化/创意/环保/医疗",
        "火": "互联网/传媒/能源/娱乐/销售",
...
# ── §13 子女 ──
def analyze_children(ri_zhu: str, gender: str, hour_gan: str, hour_zhi: str,
    ...
    return {
        "child_star": hour_ss,
        "child_count_estimate": "1-2个",
        "windows": windows if windows else ["35~45岁"],
    }

# ── §14 健康 ──
def analyze_health_detail(bazi_gans: list, bazi_zhis: list,
                           shen_label: str, liu_nian_events: list) -> dict:
    # 五行过三
    wx_count = {}
    for g in bazi_gans:
    ...
    organ_map = {"金": "肺/大肠/呼吸", "木": "肝/胆/神经", "水": "肾/膀胱/内分泌",
                 "火": "心/小肠/眼", "土": "脾/胃/消化"}
...
def generate_three_verdicts(shen_label: str, cai_score: float,
    ...
    verdicts.append({
        ...
        "degree": f"小富~中富" if cai_score < 40 else "中富以上",
...
def generate_da_yun_curve(da_yun_list: list) -> dict:
    ...
    for dy in da_yun_list:
        ...
        curve.append({
            "da_yun": dy["gan_zhi"],
            "age": f"{dy['start_age']}~{dy['end_age']}岁",
            "score": score,
            "bar": bar,
        })
    return {"curve": curve}
...
def generate_wu_xing_advice(xi_yong: list) -> dict:
    ...
    stone_map = {"金": "白金/银饰", "水": "黑曜石/海蓝宝", "木": "绿松石/翡翠", "火": "红玛瑙/石榴石", "土": "黄水晶/蜜蜡"}
    
    xi = xi_yong[0] if xi_yong else "土"
    return {
        "colors": color_map.get(xi, "白/蓝"),
        "numbers": {"lucky": "1,6(水) 4,9(金)" if xi in "金水" else "3,8(木) 2,7(火)",
...
def generate_life_advice(shen_label: str, cai_score: float,
                          xi_yong: list, ge_ju_detail: str,
                          da_yun_list: list, marriage_info: dict) -> dict:
    return {

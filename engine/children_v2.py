...
from constants import (
    TIAN_GAN, DI_ZHI, TIAN_GAN_YIN_YANG, TIAN_GAN_WU_XING,
    DI_ZHI_WU_XING, DI_ZHI_CANG_GAN, CANG_GAN_RATIO,
...
from xing_chong_he_hua import (
    ...
    check_hai, check_liu_he, check_san_he, check_ban_he,
    LIU_CHONG, SAN_XING, LIU_HAI, LIU_HE, SAN_HE, BAN_HE,
)
...
def _get_shi_shen_wu_xing(shi_shen: str) -> Optional[str]:
    """根据十神名称返回其五行克日主的反向五行。
...
def get_twelve_chang_sheng_base(
    gender: str, ri_zhu: str, hour_zhi: str,
    unified_stars: dict,
) -> dict:
    """
    计算十二长生基数。
...
def calc_children_star_score(
    ...
    if xing_list:
        score -= 10
        details.append({"项": f"地支刑损子女星", "分": -10})
...
def calc_three_layer_fate(
    fertility: dict,
    star_score: dict,
    hour_shi_shen: str,
    ri_zhu_wx_xi_ji: Optional[str],
) -> dict:
    """
    子女数量三层合参法。
...
def infer_child_birth_years(
    ...
    da_yun_list: Optional[list[dict]] = None,
    ...
    for da_yun in da_yun_list:
        ...
        end_year = da_yun.get("end_year", start_year + 9) if isinstance(da_yun, dict) else (da_yun.start_year + 9) if hasattr(da_yun, 'start_year') else start_year + 9
        ...
        for year in range(start_year, min(end_year + 1, start_year + 15)):
            ...
            # 六合
            for (z1, z2), wx in LIU_HE.items():
                if (liu_nian_zhi == z1 and hour_zhi == z2) or \
                   (liu_nian_zhi == z2 and hour_zhi == z1):
...
def check_thin_fate_factors(
    ri_zhu: str, gender: str, hour_gan: str, hour_zhi: str,
    bazi_gans: list[str], bazi_zhis: list[str],
    unified_stars: dict,
    shen_label: Optional[str] = None,
) -> dict:
    """
    子女缘薄因素排查。
    ...
    if LIU_CHONG.get(ri_zhi) == hour_zhi:
        factors.append({
            "因素": "日时冲",
            "说明": f"日支{ri_zhi}冲时支{hour_zhi}，与子女缘分薄，或子女不在身边",
            "严重": True,
        })
    ...
    if _is_yang_gan(ri_zhu):
        ...
        if hour_zhi == yang_ren_zhi.get(ri_zhu):
            factors.append({
                "因素": "时支阳刃",
                "说明": f"时支{hour_zhi}为日主{ri_zhu}之阳刃，难产之象",
                "严重": True,
            })
    ...
    if yin_count >= 2 and gender == "女":
        factors.append({
            "因素": "印旺制食伤",
            "说明": f"天干{yin_count}个印星克制食伤，不容易受孕",
            "严重": True,
        })
    
    return {
        "因素列表": factors,
        "严重因素数": sum(1 for f in factors if f["严重"]),
        "总因素数": len(factors),
    }


def check_fertility_windows(
    gender: str, ri_zhu: str,
    bazi_gans: list[str], bazi_zhis: list[str],
    hour_zhi: str,
    da_yun_list: Optional[list] = None,
...
def check_shi_zhu_zheng_guan_effect(
    ri_zhu: str, hour_gan: str, hour_zhi: str,
    bazi_gans: list[str],
    shen_label: Optional[str] = None,
) -> dict:
    """
...
def check_parent_joint_rules(
    father_ri_zhu: Optional[str],
    mother_ri_zhu: Optional[str],
    father_bazi_gans: Optional[list[str]],
    father_bazi_zhis: Optional[list[str]],
    mother_bazi_gans: Optional[list[str]],
    mother_bazi_zhis: Optional[list[str]],
...
def analyze_children_full(
    ...
    shen_label: Optional[str] = None,
    birth_year: Optional[int] = None,
    father_ri_zhu: Optional[str] = None,
    mother_ri_zhu: Optional[str] = None,
    actual_children: Optional[list[dict]] = None,
) -> dict:
    """
    子女/生育完整分析主入口。
    ...
    # ── Step 1: 扫八字子女星 ──
    star_scan = scan_children_stars_in_bazi(
        ri_zhu, bazi_gans, bazi_zhis, unified_stars["all_children_stars"]
    )
    ...
    birth_year_inference = infer_child_birth_years(
        ri_zhu, gender, hour_gan, hour_zhi,
        bazi_gans, bazi_zhis, da_yun_list, birth_year
    )
    ...
    fertility_windows = check_fertility_windows(
        gender, ri_zhu, bazi_gans, bazi_zhis, hour_zhi, da_yun_list
    )
    ...
    # ── Step 12: 时柱七杀有制 ──
    qi_sha_rule = check_shi_zhu_qi_sha_you_zhi(
        ri_zhu, gender, hour_gan, hour_zhi, bazi_gans, bazi_zhis, shen_label
    )
    
    # ── Step 13: 时柱正官影响 ──
    zheng_guan_effect = check_shi_zhu_zheng_guan_effect(
        ri_zhu, hour_gan, hour_zhi, bazi_gans, shen_label
    )

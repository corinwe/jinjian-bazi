...
# ──── 1. 引擎测试 ────
print("\n# 1/7 引擎测试（320条）")
r = subprocess.run(["python3", f"{ENGINE_DIR}/tests/test_full_suite.py"],
...
family_tests = [
    ("子源", BaZi(year=Pillar("辛","卯"), month=Pillar("壬","辰"),
                  day=Pillar("庚","戌"), hour=Pillar("辛","巳"), gender="男"),
...
paipan_tests = [
    ("2011-04-25-10", 2011, 4, 25, 10, "辛卯 壬辰 庚戌 辛巳"),
]
...
r = api_post("/api/v1/engine/debug", {"name":"测试","gender":"男","birth_year":2011,"birth_month":4,"birth_day":25,"birth_hour":10})
check("POST engine/debug", r.get("success")==True, r.get('paipan',{}).get('bazi',''))
r = api_post("/api/v1/report", {"name":"子源","gender":"男","birth_year":2011,"birth_month":4,"birth_day":25,"birth_hour":10})
...
for y,m,d in [(1980,5,15), (2011,4,25)]:
    ...
    r1 = api_post("/api/v1/engine/debug", {"name":"测试","gender":"男","birth_year":y,"birth_month":m,"birth_day":d,"birth_hour":10,"calendar_type":"lunar"})
    r2 = api_post("/api/v1/engine/debug", {"name":"测试","gender":"男","birth_year":sol.year,"birth_month":sol.month,"birth_day":sol.day,"birth_hour":10,"calendar_type":"solar"})

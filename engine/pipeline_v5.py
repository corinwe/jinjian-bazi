...
def run_v5(bazi: BaZi, birth_year=1980, birth_month_lunar=1,
    ...
    ln_results = analyze_liu_nian_range(
        all_gans, all_zhis, ri_zhu, TIAN_GAN_WU_XING[ri_zhu],
    ...
    edu = analyze_education(
        all_gans, all_zhis, ri_zhu, sqr_score, sqr_label, xi,
        dy_gans, dy_zhis, dy_ages
    )
    
    # 婚姻（v2 — 配偶星替代+四大信号+夫妻宫十神）
    mar = analyze_marriage(
        ri_zhu, bazi.day.zhi, bazi.gender, all_gans, all_zhis,
        sqr_label, sqr_score, xi, dy_gans, dy_zhis, dy_ages
    )
    ...
    fam = analyze_nian_yue(
        bazi.year.gan, bazi.year.zhi, bazi.month.gan, bazi.month.zhi,
        ri_zhu, sqr_label
    )
    ...
    comprehensive = run_comprehensive_engine(
        ...
        mar, edu, birth_year, current_year
    )
    ...
    return {
        ...
        "sec_2_ge_ju": {
            "main": main_ge,
            "detail": detail_ge,
            "shi_shen": get_shi_shen_all_dry(bazi),
        },
        ...
        "sec_5_zai_huo": {
            ...
            "wu_xing_over_three": [
                {"wx": k, "count": v}
                for k, v in compute_energy_profile(bazi)["wu_xing_energy"].items()
                if v >= 3
            ],
            "misfortune_full": analyze_misfortune(
                all_gans, all_zhis, ri_zhu, bazi.gender,
                sqr_label, sqr_score, bazi.year.zhi,
                _get_na_yin(bazi.year.gan, bazi.year.zhi),
                current_year - birth_year
            ),
            "remission_advice": analyze_remission(xi, ji,
                analyze_misfortune(all_gans, all_zhis, ri_zhu, bazi.gender,
                    sqr_label, sqr_score, bazi.year.zhi,
                    _get_na_yin(bazi.year.gan, bazi.year.zhi),
                    current_year - birth_year)["risk_level"]
            ),
        },
...
def format_21_section_report(result: dict) -> str:
    ...
    det = s3.get("details", {})
    lines.append(f"  月令印: {det.get('yue_yin',0)} | 月令比劫: {det.get('yue_bi',0)} | 天干比劫: {det.get('tg_bi',0)} | 日支: {det.get('rz',0)} | 年时: {det.get('nsz',0)} | 总分: {det.get('total',0)}")
    lines.append("")
    ...
    if isinstance(s12, dict):
        lines.append(f"## §12 婚姻: {s12.get('quality','')}({s12.get('quality_score','')}/10) | 最佳{s12.get('best_window_age','')}")
...
def run_pipeline(name: str, gender: str,
    ...
    strongest_wx = max(energy_profile.get("wu_xing_energy", {}).items(), key=lambda x: x[1])[0] if energy_profile.get("wu_xing_energy") else ""
    weakest_wx = min(energy_profile.get("wu_xing_energy", {}).items(), key=lambda x: x[1])[0] if energy_profile.get("wu_xing_energy") else ""
    ...
    output = {
        ...
        "analysis": {
            ...
            "dimensions": {n: {"base": ds.base, "da_yun_bonus": ds.da_yun_bonus, "total": ds.total} for n, ds in dims_raw.items()},
        },
        "result": result,
...
if __name__ == "__main__":
    ...
    output = run_pipeline(args.name, args.gender, yg, yz, mg, mz, dg, dz, hg, hz, calc_year, birth_month_lunar, qi_yun_days)
    output["success"] = True

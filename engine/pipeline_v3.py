...
from constants import BaZi, Pillar, TIAN_GAN_WU_XING, DI_ZHI_WU_XING
from shi_shen import get_shi_shen_all_dry, get_all_cang_gan_shi_shen
...
def run_v3(bazi: BaZi,
    ...
    shi_shen = {
        "tian_gan": get_shi_shen_all_dry(bazi),
        "cang_gan": get_all_cang_gan_shi_shen(bazi),
    }
    ...
    shen_sha = compute_all_shen_sha(all_gans, all_zhis,
                                     bazi.year.zhi, bazi.month.zhi, ri_zhu)
    ...
    liu_nian_results = analyze_liu_nian_range(
        all_gans, all_zhis,
        ri_zhu, TIAN_GAN_WU_XING[ri_zhu],
        bazi.year.zhi, bazi.month.zhi,
        dy_gans, dy_zhis, dy_start_years,
        sqr_score, sqr_label,
        xi, ji,
    ...
    education = analyze_education(
        all_gans, all_zhis, ri_zhu,
        sqr_score, sqr_label, xi,
        dy_gans, dy_zhis, dy_start_ages,
    )
    ...
    marriage = analyze_marriage(
        ri_zhu, bazi.day.zhi, bazi.gender,
        all_gans, sqr_label, sqr_score, xi,
        dy_gans, dy_zhis, dy_start_ages,
    )
    ...
    return {
        ...
        # 流年
        "liu_nian": {
            "recent_5": liu_nian_results[:5],
            "key_events": {k: v for k, v in key_events.items() if v},
        },
...
if __name__ == "__main__":
    ...
    for name, yg,yz,mg,mz,dg,dz,hg,hz,gn,by,bm in test_cases:
        bazi = BaZi(year=Pillar(yg,yz), month=Pillar(mg,mz),
                    day=Pillar(dg,dz), hour=Pillar(hg,hz), gender=gn)

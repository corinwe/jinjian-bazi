...
from constants import BaZi, Pillar, TIAN_GAN_WU_XING, DI_ZHI_WU_XING, NA_YIN
from shi_shen import get_shi_shen_all_dry, get_all_cang_gan_shi_shen, get_shi_shen_for_gan
...
def run_v4(bazi: BaZi, birth_year=1980, birth_month_lunar=1,
           qi_yun_days=1.1, current_year=None):
    ...
    ln_results = analyze_liu_nian_range(all_gans, all_zhis, ri_zhu, TIAN_GAN_WU_XING[ri_zhu],
        bazi.year.zhi, bazi.month.zhi, dy_gans, dy_zhis, dy_years,
        sqr_score, sqr_label, xi, ji, birth_year, current_year, 30)
    ...
    mar = analyze_marriage(ri_zhu, bazi.day.zhi, bazi.gender, all_gans, sqr_label, sqr_score, xi, dy_gans, dy_zhis, dy_ages)
    dims = {n:{"base":ds.base,"da_yun_bonus":ds.da_yun_bonus,"total":ds.total} for n,ds in DEFAULT_DIMENSIONS(bazi).items()}
    
    # 新增模块
    app = analyze_appearance(ri_zhu, sqr_label, sqr_score, all_gans, all_zhis)
    prop = analyze_property(ri_zhu, bazi.day.zhi, xi,
        [{"gan_zhi":d.gan_zhi,"score":dy_scores[i][1],"start_age":d.start_age,"end_age":d.end_age} for i,d in enumerate(dy_list)], best_idx)
    car = analyze_career_detail(ri_zhu, all_gans, sqr_label, xi, detail_ge)
    child = analyze_children(ri_zhu, bazi.gender, bazi.hour.gan, bazi.hour.zhi,
        [{"start_age":d.start_age,"end_age":d.end_age,"score":dy_scores[i][1]} for i,d in enumerate(dy_list)])
    health = analyze_health_detail(all_gans, all_zhis, sqr_label,
        [e for evts in key_evts.values() for e in evts])
    best_dy = {"gan_zhi":dy_list[best_idx].gan_zhi,"start_age":dy_list[best_idx].start_age,"end_age":dy_list[best_idx].end_age} if best_idx>=0 else {}
    verdicts = generate_three_verdicts(sqr_label, cai.total, detail_ge, best_dy, mar)
    dy_curve = generate_da_yun_curve(
        [{"gan_zhi":d.gan_zhi,"start_age":d.start_age,"end_age":d.end_age,"score":dy_scores[i][1]} for i,d in enumerate(dy_list)])
    wx_advice = generate_wu_xing_advice(xi)
    advice = generate_life_advice(sqr_label, cai.total, xi, detail_ge,
        [{"gan_zhi":d.gan_zhi,"start_age":d.start_age} for d in dy_list], mar)
    ...
    return {
        ...
        "sec_1_overview": {
            "bazi": bazi.summary(),
            "na_yin": [NA_YIN.get(f"{bazi.year.gan}{bazi.year.zhi}",""),
            ...
            "xi_yong": xi, "ji_shen": ji,
            "cai_xing_score": cai.total,
            "cai_xing_detail": {"yue":cai.yue_ling_score,"ri":cai.ri_zhi_score,"shi_gan":cai.shi_gan_score,"shi_zhi":cai.shi_zhi_score,"nian":cai.nian_zhi_score},
        ...
        # §3 身强弱
        "sec_3_shen_qiang_ruo": {"score":sqr_score,"label":sqr_label,
            "details":{"yue_yin":sqr_detail.yue_ling_yin,"yue_bi":sqr_detail.yue_ling_bi_jie,
                       "tg_bi":sqr_detail.tian_gan_bi_jie,"rz":sqr_detail.ri_zhi_yin_bi,
                       "nsz":sqr_detail.nian_shi_zhi_yin_bi,"total":sqr_detail.total}},
        # §4 喜用神
        "sec_4_xi_yong": {"xi":xi,"ji":ji,"tiao_hou":th},
        # §5 灾祸/搬迁
        "sec_5_za_i_hui": {
            "shen_sha_chong": dz["冲"], "shen_sha_xing": [x["type"] for x in dz["刑"]],
            "shen_sha_hai": dz["害"], "yuan_chen": ss.yuan_chen,
            "zai_sha": ss.zai_sha, "tian_luo": ss.tian_luo_di_wang,
            "wu_xing_over_three": [{"wx":k,"count":v} for k,v in compute_energy_profile(bazi)["wu_xing_energy"].items() if v>=3],
        },
        # §6 性格
        ...
        "sec_8_wealth": {
            ...
            "cai_xing_details": {"nian":cai.nian_zhi_score,"yue":cai.yue_ling_score,"ri":cai.ri_zhi_score,"sg":cai.shi_gan_score,"sz":cai.shi_zhi_score},
            "cai_ku": {"has": any(z in "辰戌丑未" for z in [bazi.day.zhi,bazi.hour.zhi]),
        ...
        "sec_17_da_yun_detail": {
            "list": [{"gan_zhi":d.gan_zhi,"start_age":d.start_age,"end_age":d.end_age,
                      "start_year":d.start_year,"score":dy_scores[i][1],
                      "gan_ss":get_shi_shen_for_gan(d.gan,ri_zhu)} for i,d in enumerate(dy_list)],
...
def format_21_section_report(result: dict) -> str:
    ...
    lines.append(f"## §8 财富: {s8['cai_xing_total']}分 | 等级: {s8['wealth_level']} | 财库: {'有' if s8['cai_ku']['has'] else '无'}")
    lines.append(f"")
    ...
    lines.append(f"## §19 运程曲线")
    for c in s19["curve"]:
        lines.append(f"  {c['da_yun']}({c['age']}): {c['bar']} {c['score']}/10")
    lines.append(f"")
    
    lines.append(f"---")
    lines.append(f"金鉴真人·全量报告引擎 v4.0 | 21§完整输出")

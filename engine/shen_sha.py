...
def get_wen_chang(gan: str) -> str | None:
    """获取文昌贵人"""
    return WEN_CHANG.get(gan)


def get_yi_ma(zhi: str) -> str | None:
    """获取驿马"""
    return YI_MA.get(zhi)


def get_tao_hua(zhi: str) -> str | None:
    """获取桃花"""
    return TAO_HUA.get(zhi)


def get_hua_gai(zhi: str) -> str | None:
    """获取华盖"""
    return HUA_GAI.get(zhi)


def check_tian_luo_di_wang(zhi: str) -> str | None:
    """检查天罗地网"""
    if zhi in TIAN_LUO:
        return "天罗"
    if zhi in DI_WANG:
...
def compute_all_shen_sha(
    bazi_gan_list: list[str], bazi_zhi_list: list[str], year_zhi: str, month_zhi: str, ri_gan: str
) -> ShenShaResult:
    """
    计算全部神煞
    参数:
    ...
    parts = []
    if result.tian_yi:
        parts.append(f"天乙: {','.join(result.tian_yi)}")
    if result.wen_chang:
        parts.append(f"文昌: {','.join(result.wen_chang)}")
    if result.yi_ma:
        parts.append(f"驿马: {','.join(result.yi_ma)}")
    if result.tao_hua:
        parts.append(f"桃花: {','.join(result.tao_hua)}")
    if result.hua_gai:
        parts.append(f"华盖: {','.join(result.hua_gai)}")
    if result.tian_luo_di_wang:
        parts.append(f"天罗地网: {','.join(result.tian_luo_di_wang)}")

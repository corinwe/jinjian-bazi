...
from xing_chong_he_hua import (
    ...
    check_hai, check_liu_he, check_san_he,
    NENG_LIANG as REL_ENERGY,
)
from shen_sha import (
    compute_all_shen_sha, ZAI_SHA, YUAN_CHEN,
    SHEN_SHA_JIE_ZAI,
)
...
def analyze_liu_nian_v2(
    ...
    shen_sha_full = compute_all_shen_sha(
        all_gans + [liu_nian_gan],
        all_zhis + [liu_nian_zhi],
        year_zhi, month_zhi, ri_zhu
    )
    ...
    if wealth_conf >= 0.5:
        ...
        events.append(EventPrediction(
            year=year, event_type="wealth",
            description=f"💰 发财窗口: {wealth_desc}",
    ...
    if mis_conf >= 0.5:
        ...
        events.append(EventPrediction(
            year=year, event_type="misfortune",
            description=f"⚠️ 注意防范: {mis_desc}",
            confidence=mis_conf,
        ))
    ...
    if mar_conf >= 0.5:
        ...
        events.append(EventPrediction(
            year=year, event_type="marriage",
            description=f"💍 姻缘窗口: {mar_desc}",
            confidence=mar_conf,
        ))
    ...
    if car_conf >= 0.4:
        events.append(EventPrediction(
            year=year, event_type="career",
            description=f"💼 事业窗口: {car_desc}",
            confidence=car_conf,
        ))
    ...
    if edu_conf >= 0.5:
        events.append(EventPrediction(
            year=year, event_type="education",
            description=f"📚 学业窗口: 印星到位+文昌引动",
            confidence=edu_conf,
        ))
    ...
    if health_conf >= 0.5:
        events.append(EventPrediction(
            year=year, event_type="health",
            description="🏥 注意健康: 五行过三/七杀攻身",
...
def extract_key_events(liu_nian_results: list[dict]) -> dict:
    """提取关键事件年表"""
    events = {"wealth": [], "misfortune": [], "marriage": [],

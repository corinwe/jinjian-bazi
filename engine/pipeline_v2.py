...
from liu_nian import analyze_liu_nian, get_liu_nian_gan_zhi, generate_liu_nian_years


def run_v2(bazi: BaZi,
           birth_year: int = 1980,
    ...
    for year in generate_liu_nian_years(birth_year, current_year, 10):
        ...
        ln = analyze_liu_nian(lg, lz, ri_zhu, TIAN_GAN_WU_XING[ri_zhu],
                              bazi.year.zhi, dy_gan, dy_zhi, all_zhis)
        ln["year"] = year
...
if __name__ == "__main__":
    ...
    for name, yg,yz,mg,mz,dg,dz,hg,hz,gn,by,bm in test_cases:
        ...
        for ln in result["liu_nian"][:3]:
            print(f"  {ln['year']} {ln['流年']}: {ln['流年十神']} 评分{ln['综合评分']}/10 {'⚠️'+ln['犯太岁']['类型'] if ln['犯太岁'] else ''}")

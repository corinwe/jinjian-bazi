...
def main():
    ...
    mode = _mode()
    ds_env = os.environ.get("BAZI_DATASOURCE", "")
    name = _extract_name(filepath, ds_env)

    # ── 八字侧门禁（dual / bazi 模式）──
    # 🚨 2026-09-10 修复：不再依赖 BAZI_DATASOURCE 环境变量
    #    （hook 是独立子进程，agent 会话里 export 的变量传不进来 → 旧逻辑必然误拦/漏拦）。
    #    改为**直接校验磁盘产物**：优先 env 指定，其次按姓名找 /tmp/{姓名}_ds.json。
    if mode in ("dual", "bazi"):
        ds_path = ""
        if ds_env and os.path.exists(ds_env):
            ds_path = ds_env
        else:
            cands = []
            if name:
                cands += [f"/tmp/{name}_ds.json", f"/tmp/{name}_engine.json"]
            for c in cands:
                if os.path.exists(c):
                    ds_path = c
                    break
            if not ds_path:
                import glob
                g = sorted(glob.glob("/tmp/*_ds.json"), key=os.path.getmtime, reverse=True)
                if len(g) == 1:
                    ds_path = g[0]
        if not ds_path:
            _block(
                f"⛔ 物理拦截【双引擎门禁】：未找到八字数据源。\n"
                f"  期望文件: /tmp/{name or '{姓名}'}_ds.json\n"
                f"  唯一入口（同时产出八字+紫微双源）:\n"
                f"  bash projects/bazi-platform/scripts/bazi-dual-prepare.sh {name or '<姓名>'} <性别> <YYYY-MM-DD> <HH:MM> [出生地]\n"
                f"  然后: touch /tmp/.bazi_verified"
            )
            return

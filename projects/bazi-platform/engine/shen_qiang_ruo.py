...
核心规则（强制·不可违背）:
  ...
  5. 燥土规则: 未/戌 + 天干丙/丁引化 = 当火看（不计印分、不生金）
              未/戌 + 天干壬/癸灭火 = 当土看（生金）
  6. 从弱恒定50分（从弱格恒定50分）
  7. 阈值: >60=身强, 40~60=中和, <40=身弱, ≤0=从弱(或自坐比劫永不从弱→身弱)
...
from constants import CANG_GAN_RATIO, TIAN_GAN_WU_XING, WU_XING_SHENG, BaZi, Pillar, ScoreDetails
...
def compute_shen_qiang_ruo(bazi: BaZi) -> tuple[float, str, ScoreDetails]:
    ...
    # ═══════════════════════════════════════
    total = (
        details.yue_ling_yin
        + details.yue_ling_bi_jie
        + details.tian_gan_yin
        + details.tian_gan_bi_jie
        + details.ri_zhi_yin_bi
        + details.nian_shi_zhi_yin_bi
    )
    total = round(total, 1)
    details.total = total
...
if __name__ == "__main__":
    ...
    # 测试1: 家主 甲午 己巳 戊午 壬子
    bazi1 = BaZi(
        year=Pillar("甲", "午"), month=Pillar("己", "巳"), day=Pillar("戊", "午"), hour=Pillar("壬", "子"), gender="男"
    )
    ...
    # 测试3: 主母 戊午 甲子 庚戌 丁亥
    bazi3 = BaZi(
        year=Pillar("戊", "午"), month=Pillar("甲", "子"), day=Pillar("庚", "戌"), hour=Pillar("丁", "亥"), gender="女"
    )
    ...
    # 测试4: 父亲 己丑 癸酉 癸亥 戊午
    bazi4 = BaZi(
        year=Pillar("己", "丑"), month=Pillar("癸", "酉"), day=Pillar("癸", "亥"), hour=Pillar("戊", "午"), gender="男"
    )

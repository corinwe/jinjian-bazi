...
def run_paipan(name: str, gender: str, year: int, month: int, day: int, hour: int, minute: int = 0) -> dict:
    """Step 1: 排盘"""
    if not os.path.exists(BAZI_ENGINE):
    ...
    try:
        ...
        return {
            ...
            "ri_zhu": ri_zhu,
            "hour_idx": hour_idx,
            "shi_chen": SHI_CHEN_NAMES.get(hour_idx, ""),
        }
...
def full_analysis(
    ...
    result["paipan"] = {"bazi": paipan["bazi"], "shi_chen": paipan["shi_chen"], "ri_zhu": paipan["ri_zhu"]}

    # ═══ Step 2: 11行×4列基础数据 ═══
    basic = compute_basic_data(
        year_gan=paipan["year_gan"],
        year_zhi=paipan["year_zhi"],
        month_gan=paipan["month_gan"],
        month_zhi=paipan["month_zhi"],
        day_gan=paipan["day_gan"],
        day_zhi=paipan["day_zhi"],
        hour_gan=paipan["hour_gan"],
        hour_zhi=paipan["hour_zhi"],
        gender=gender,
    ...
    analysis["xi_yong_shen"] = {"xi": xi, "ji": ji, "tiao_hou": tiao_hou}
    ...
    analysis["da_yun"] = {"qi_yun_age": round(qi_yun_age, 2), "list": []}
    for dy in da_yun_list:
        end_year = dy.start_year + 9
        analysis["da_yun"]["list"].append(
            {"gan_zhi": dy.gan + dy.zhi, "start_year": dy.start_year, "end_year": end_year}
        )
    ...
    if liu_nian_gan_zhi:
        ...
        liu_nian = analyze_liu_nian(
            liu_nian_gan, liu_nian_zhi, bazi.ri_zhu, bazi.ri_zhu_wu_xing, bazi.year.zhi, dy_gan, dy_zhi, all_zhi
        )
    else:
